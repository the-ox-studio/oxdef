...
export class TemplateExpander {
  constructor(transaction, dataSourceProcessor, macroContext = null) {
  ...
  expandTemplates(document) {
    ...
    // Also process regular blocks (through expandNodes to trigger onWalk)
    if (document.blocks && document.blocks.length > 0) {
      const processedBlocks = this.expandNodes(document.blocks, null);
      expandedBlocks.push(...processedBlocks);
    }
  ...
  expandOnData(onDataNode) {
    ...
    if (this.dataSourceProcessor.isDataSourceSuccessful(sourceName)) {
      ...
      try {
        ...
        const expandedBlocks = this.expandNodes(onDataNode.dataBlocks, null);
    ...
    if (this.dataSourceProcessor.isDataSourceFailed(sourceName)) {
      ...
      try {
        ...
        const expandedBlocks = this.expandNodes(onDataNode.errorBlocks, null);
  ...
  expandIf(ifNode) {
    ...
    if (this.expressionEvaluator.toBoolean(condition)) {
      ...
      return this.expandNodes(ifNode.thenBlocks, null);
    }
    ...
    if (ifNode.elseIfBranches && ifNode.elseIfBranches.length > 0) {
      for (const branch of ifNode.elseIfBranches) {
        ...
        if (this.expressionEvaluator.toBoolean(elseifCondition)) {
          return this.expandNodes(branch.blocks, null);
        }
      }
    }
    ...
    if (ifNode.elseBlocks && ifNode.elseBlocks.length > 0) {
      return this.expandNodes(ifNode.elseBlocks, null);
    }
  ...
  expandForeach(foreachNode) {
    ...
    try {
      ...
      for (let i = 0; i < collection.length; i++) {
        ...
        const iterationBlocks = this.expandNodes(clonedBody, null);
